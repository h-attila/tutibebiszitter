...
{
    protected string $adminProfileFormType;
    protected string $profileGroup;
...
//        protected LoggerInterface        $logger,
     ...
     */
    public function renewNow(string $uuid): Response
    {
     ...
     */
    public function save(string $uuid, Request $request, ValidatorInterface $validator, StatusService $statusService, UserPasswordHasherInterface $userPasswordHasher): Response
    {
        $p = $this->em->getRepository(Profile::class);
        $now = new DateTime();
        $errors = [];
        ...
        $form = $this->createForm($this->adminProfileFormType, $profile);
        $form->submit($data);
        ...
        if ($form->isValid()) {
            ...
            if (!empty($data['places'])) {
                ...
                $districts = $this->em->getRepository(Place::class)->getBudapestDistricts();
                /* @var Place $budapestOsszes */
                $budapestOsszes = $this->em->getRepository(Place::class)->findOneBy(['cityCode' => 'bp_osszes']);

                foreach ($allowedPlaces as $allowedPlace) {
                    $found = false;
                    foreach ($data['places'] as $place) {
                        if ($budapestOsszes instanceof Place && $budapestOsszes->getId() === $place['id']) {
                            foreach ($districts as $district) {
                                $profile->addPlace($district);
                            }
                        } elseif ($allowedPlace->getId() === $place['id']) {
            ...
            $json = $this->serializer->serialize($profile, 'json', ['groups' => [$this->profileGroup]]);
